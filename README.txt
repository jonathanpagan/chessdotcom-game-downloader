...
As a competitive chess player, reviewing my games is instrumental in determining my weaknesses. Chess.com is the most popular server for playing chess, featuring the strongest competition. 
...
The script scrapes the Chess.com API to get a user's games in json form and load each game into a Pandas DataFrame, from where the games can be filtered (i.e. filtering out games played with non-standard rules). The output is a .pgn file, a text file that can be opened in chess software to play through games.
